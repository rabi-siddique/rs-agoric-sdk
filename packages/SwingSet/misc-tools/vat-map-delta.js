...
//
// If given a third argument, a JSON-formatted change report is
// written to that file.

const [previousFn, newFn, changesFn] = process.argv.slice(2);
const oldData = JSON.parse(fs.readFileSync(previousFn));
const newData = JSON.parse(fs.readFileSync(newFn));
...
if (keys(changes).length) {
  ...
  if (changesFn) {
    fs.writeFileSync(changesFn, `${JSON.stringify(changes)}\n`);
  }
}

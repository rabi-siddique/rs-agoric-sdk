...
// run this like:
//  node vat-map-from-swingstore.js run-04-swingstore-pruned.sqlite vat-map-04.json 04
//
...
const [swingstoreDBfn, outputFn, runID] = process.argv.slice(2);
const ssdb = sqlite3(swingstoreDBfn);
...
const vatNumber = vatID => idNumber('v', vatID); // 'v12' -> 12
...
sortWith(allVatIDs, vatNumber);
// console.log(allVatIDs);

const knownZCFIDs = [
  // established at launch, aka namedBundleID.zcf :
  ...
  // TODO added when?? appears in run-30 but not run-01
...
// we happen to know how device-vat-admin manages its state
...
const kdNumber = vatID => Number(vatID.slice(1)); // 'v12' -> 12
const importedKdids = vatID => {
  ...
  sortWith(kdids, kdNumber);
  return kdids;
};
const lastImportedKdid = vatID => importedKdids(vatID).at(-1);

const output = { runID, currentZCF: undefined, vats: {} };
...
for (const vatID of allVatIDs) {
  ...
  const { incarnation, endPos } = sqlGetSpan.get(vatID);
